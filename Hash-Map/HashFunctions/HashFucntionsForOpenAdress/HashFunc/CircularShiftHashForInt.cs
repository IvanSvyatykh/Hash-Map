    ...
    internal class CircularShiftHashForInt : IOpenAdressHashFunc
    {
        ...
        private int HashFunc(object data, int size, int attemptNumber)
        {
           ...
           int key = ((int)data);
            ...
            key += ~(key << 16);
            key ^= (key >> 5+attemptNumber);
            key += (key << 3);
            key ^= (key >> 13 + attemptNumber);
            key += ~(key << 9);
            key ^= (key >> 17 + attemptNumber);

            return key%size;

            //string key = ((int)data).ToString();
            ...
            //return (hash+attemptNumber)%size;
        }
